...
VI_ZVK_VD_VS2_NOOPERANDS_PRELOOP_EGU32x4_NOVM_LOOP(
  ...
  {},  // No PRELOOP.
  {
    ...
    const EGU8x16_t plaintext = P.VU.elt_group<EGU8x16_t>(vd_num, idx_eg);
    const EGU8x16_t orig_key = P.VU.elt_group<EGU8x16_t>(vs2_num, idx_eg);

    EGU8x16_t aes_state = plaintext;

    DLOG("vaese128: @start"
         " plaintext(%" PRIxEG "), key(%" PRIxEG ") idx_eg(%" PRIuREG ")",
    ...
    // AddRoundKey
    DLOG("vaesef: @k_sch round_key(%" PRIxEG ")", PRVEG(round_key));
    EGU8x16_XOREQ(aes_state, round_key);
